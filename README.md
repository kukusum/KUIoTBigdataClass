...
- pointer & array
    ...
    - p = nums; (?)
    - 포인터와 배열의 관계
        - 배열이름이 식에서 사용되면 첫번째 엘리먼트의 시작주소를 의미한다
        - //한 주소당 4바이트씩 사용한다. (?)
        - p = p + 1; == ++p;
        - 포인터의 더하기1은 1바이트값을 더함?
        - char형 포인터 double형 포인터에 +1은 각각 값이 다름
        - 포인터의 +1은 다음 엘리먼트를 가르킨다. ***
        - 포인터에 더하는 값은 배열에서 사용하는 INDEX !!
        - +1은 주소값에 1을 더하는게 아니고 다음 엘리먼트
